...
export function useCardSelection({ roomCode, round, userId }: Params) {
  ...
  const [slots, setSlots] = useState<HandSlot[]>(INITIAL_HAND_SLOTS)
  ...
  // Covers the full async flow so double-taps are blocked even in the window
  // between generate() returning and insertCard() completing.
